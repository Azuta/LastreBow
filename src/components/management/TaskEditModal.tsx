...
const TaskEditModal = ({ isOpen, onClose, onSave, task, members }: TaskEditModalProps) => {
    ...
    // Usar useEffect para sincronizar el estado del modal con la tarea actual
    useEffect(() => {
        if (task) {
    ...
    const handleSave = () => {
        onSave({ ...task, title: taskTitle, assignedTo, color });
    };
            ...
            <div className="bg-[#201f31] rounded-lg shadow-lg w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex justify-end gap-4 pt-4">
                        ...
                        <button type="button" onClick={handleSave} className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700">Guardar</button>
                    </div>
                </div>
            </div>
        </div>
