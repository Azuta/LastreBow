...
const MangaListCard = ({ list }: { list: UserList }) => {
    // Aseguramos que el usuario exista en el objeto para evitar errores
    const username = list.user?.username || 'unknown';

    return (
        // El Link ahora apunta a la página de detalles de la lista
        <Link href={`/user/${username}/list/${list.id}`}>
