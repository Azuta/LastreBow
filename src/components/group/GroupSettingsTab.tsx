...
import RecruitmentPermissionsModal from './RecruitmentPermissionsModal';
            ...
            <div className="max-w-screen-md mx-auto space-y-8 py-8">
                {hasChanges && (
                    <div className="sticky top-20 z-10 p-4 bg-[#2b2d42]/90 backdrop-blur-sm rounded-lg border border-gray-700 flex justify-between items-center">
                        ...
                        <div className="flex gap-4">
                            <button onClick={handleDiscard} className="flex items-center gap-2 px-4 py-2 bg-gray-600/80 text-gray-300 rounded-lg text-sm font-semibold hover:bg-gray-700 transition-colors"><UndoIcon /> Descartar</button>
                            <button onClick={handleSave} disabled={isSaving} className="flex items-center gap-2 px-6 py-2 bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-700 transition-colors disabled:opacity-50"><SaveIcon /> {isSaving ? 'Guardando...' : 'Guardar Cambios'}</button>
                        </div>
                    </div>
                )}
                ...
                <section>
                    ...
                    <div className="space-y-4">
                        <SettingsSwitch label="Tablero Kanban Público" description="Permite que cualquier persona (no miembros) vea el estado de tus proyectos en el tablero Kanban." checked={settings.kanbanIsPublic} onChange={() => setSettings(prev => ({ ...prev, kanbanIsPublic: !prev.kanbanIsPublic }))} disabled={!isAdmin} />
                        <SettingsSwitch label="Permitir Sugerencias de Proyectos" description="Si está activado, los usuarios podrán sugerir mangas para que tu grupo los traduzca." checked={settings.allowProjectProposals} onChange={() => setSettings(prev => ({ ...prev, allowProjectProposals: !prev.allowProjectProposals }))} disabled={!isAdmin}/>
                        
                        <div className="bg-gray-700/30 p-4 rounded-lg flex justify-between items-center">
                             <div>
                                <h4 className="font-semibold text-white">Gestionar Permisos de Reclutamiento</h4>
                                <p className="text-sm text-gray-400">Define qué miembros pueden ver y gestionar las aplicaciones de reclutamiento.</p>
                            </div>
                            <button onClick={() => setIsPermissionsModalOpen(true)} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-700">
                                Abrir
                            </button>
                        </div>
                    </div>
