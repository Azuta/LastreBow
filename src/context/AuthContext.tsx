    ...
    useEffect(() => {
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
            ...
            if (session?.user) {
                ...
                // Si el evento es un inicio de sesión y el perfil indica que el onboarding no se ha completado,
...
setFollowedScanGroups(followedGroupsData?.map(group => group.group_id) || []);
    ...
    const signUpWithEmail = async (username: string, email: string, password: string): Promise<boolean> => {
        ...
        // Con la confirmación de email desactivada en Supabase, esto iniciará sesión automáticamente
        // y el listener onAuthStateChange se encargará de la redirección.
        return true;
    };

    const updateUserProfile = async (updates: ProfileUpdate): Promise<boolean> => {
        if (!user) return false;
        // Se elimina la restricción de correo verificado para esta función específica
    ...
    const resendVerificationEmail = async () => {
        ...
        else addToast('Correo de verificación reenviado.', 'success');
    };

    const toggleFavorite = async (media: Media) => {
        if (!user) {
            addToast('Debes iniciar sesión para añadir a favoritos.', 'error');
            return;
        }
    ...
    if (!user) {
        addToast('Debes iniciar sesión para seguir a un grupo.', 'error');
        return;
    }
