...
const HeartIcon = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>;
...
const MOCK_USERS: { [key: string]: { followedIds: number[] } } = {
  "Dymedis": { followedIds: [30002, 105778, 30013, 120980, 105398] },
  // Podrías añadir más usuarios aquí
};
...
const BrowseAllPage = () => {
  ...
  const [isViewingSharedList, setIsViewingSharedList] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (showOnlyFollowed && user.loggedIn) {
        ...
        shareParams.set('followedBy', user.username);
        shareableUrl = `${window.location.origin}${pathname}?${shareParams.toString()}`;
    }
    setUrlToShare(shareableUrl);
  ...
  const resetFilters = () => { if(!isViewingSharedList) { setSelectedGenres([]); setSelectedFormats([]); setSelectedStatus([]); }};
      ...
      <>
          ...
          <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              <header className="mb-8">
                  ...
                  <h1 className="text-4xl font-bold text-white">Explorar Mangas</h1>
              </header>
              
              {isViewingSharedList && (
                  <div className="bg-indigo-900/50 border border-indigo-700 text-indigo-300 px-4 py-3 rounded-lg mb-8 text-center text-sm">
                      Estás viendo la lista de mangas seguidos por <span className="font-bold text-indigo-200">{isViewingSharedList}</span>. <Link href="/browse/all" className="underline hover:text-white">Ver todos</Link>
                  </div>
              )}

              <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-[#201f31] p-4 rounded-lg mb-8">
                <input type="text" placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} disabled={!!isViewingSharedList} className="w-full md:w-80 bg-gray-700/50 text-gray-300 rounded-full px-4 py-2 text-base focus:outline-none focus:ring-2 focus:ring-[#ffbade] disabled:opacity-50" />
                <div className="flex items-center gap-2">
                    {user.loggedIn && (
                      <button 
                        onClick={() => setShowOnlyFollowed(!showOnlyFollowed)}
                        disabled={!!isViewingSharedList}
                        className={`p-2 rounded-full transition-colors ${showOnlyFollowed ? 'bg-[#ffbade] text-black' : 'bg-gray-700/50 hover:bg-gray-600/80 text-gray-300'} disabled:opacity-50 disabled:cursor-not-allowed`} 
                        aria-label="Mostrar solo seguidos"
                      >
                        <HeartIcon />
                      </button>
                    )}
                    ...
                    <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOptionsKey)} disabled={!!isViewingSharedList} className="bg-gray-700/50 text-gray-300 rounded-full px-4 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#ffbade] disabled:opacity-50">
